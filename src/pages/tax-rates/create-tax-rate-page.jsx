...
import {createTaxRate, selectTaxRates} from "../../redux/features/tax-rates/tax-rates-slice";
...
const CreateTaxRatePage = () => {
    ...
    const {taxRateLoading, taxRateError} = useSelector(selectTaxRates);

    const formik = useFormik({
        initialValues: {name: "", country: "", state: "", postcode: "", city: "", rate: "", tax_class: "standard", priority: 1, compound: false, shipping: true},
        validationSchema: Yup.object({
            name: Yup.string().required("Name is required"),
            rate: Yup.number().required("Rate is required").min(0),
        }),
        onSubmit: async (values) => {
            const result = await dispatch(createTaxRate({...values, rate: Number(values.rate), priority: Number(values.priority)}));
            if (!result.error) navigate("/tax-rates");
        }
    });
            ...
            <Box sx={{pt: 4, pb: 6}}>
                <Container>
                    <Stack direction="row" spacing={2} alignItems="center" sx={{mb: 3}}>
                        ...
                        <Typography variant="h5">Add Tax Rate</Typography>
                    </Stack>
                        ...
                        <Grid container spacing={3}>
                            <Grid size={{xs: 12, md: 8}}>
                                <Paper elevation={0} sx={{p: 3}}>
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid size={{xs: 12}}>
                                            <FormControlLabel control={<Checkbox name="compound" checked={formik.values.compound} onChange={formik.handleChange}/>} label="Compound (tax applied on top of other taxes)"/>
                                        </Grid>
                                        <Grid size={{xs: 12}}>
                            ...
                            <Grid size={{xs: 12, md: 4}}>
                                ...
                                <Paper elevation={0} sx={{p: 3}}>
                                    <Divider sx={{mb: 2}}/>
                                    <Stack spacing={1}>
                                        <Button type="submit" variant="contained" color="secondary" fullWidth disabled={taxRateLoading}>Save Tax Rate</Button>
