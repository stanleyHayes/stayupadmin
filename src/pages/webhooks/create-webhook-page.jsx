...
import {createWebhook, selectWebhooks} from "../../redux/features/webhooks/webhooks-slice";
...
const CreateWebhookPage = () => {
    ...
    const {webhookLoading, webhookError} = useSelector(selectWebhooks);

    const formik = useFormik({
        initialValues: {name: "", topic: "order.created", delivery_url: "", status: "active"},
        validationSchema: Yup.object({
            name: Yup.string().required("Name is required"),
            delivery_url: Yup.string().url("Must be a valid URL").required("Delivery URL is required"),
        }),
        onSubmit: async (values) => {
            const result = await dispatch(createWebhook(values));
            if (!result.error) navigate("/webhooks");
        }
    });
            ...
            <Box sx={{pt: 4, pb: 6}}>
                <Container>
                    <Stack direction="row" spacing={2} alignItems="center" sx={{mb: 3}}>
                        ...
                        <Typography variant="h5">Add Webhook</Typography>
                    </Stack>
                        ...
                        <Grid container spacing={3}>
                                    ...
                                    <Grid container spacing={2}>
                                        ...
                                        <Grid size={{xs: 12}}>
                                            <TextField
                                                ...
                                                size="small" fullWidth placeholder="https://example.com/webhook"
                                            />
                                        </Grid>
                                    </Grid>
                                </Paper>
                            ...
                            <Grid size={{xs: 12, md: 4}}>
                                ...
                                <Paper elevation={0} sx={{p: 3}}>
                                    <Divider sx={{mb: 2}}/>
                                    <Stack spacing={1}>
                                        <Button type="submit" variant="contained" color="secondary" fullWidth disabled={webhookLoading}>Save Webhook</Button>
